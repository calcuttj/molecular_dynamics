...
subroutine calc_force(position, force, length, N,temp,pressure)
  ...
  real(8) :: separation(3), sep_squared,sep, f
  integer counter1, counter2
  real(8) :: total=0
  separation = 0d0
  force = 0d0
  ...
  do counter1 = 1, N-1
     do counter2 = counter1 + 1, N
        ...
        if (sep_squared < (3.2**2)) then
           ...
           f = (48/(sep_squared**7)-24/(sep_squared**4))
           force(:,counter1) = force(:,counter1) + f*separation
           force(:,counter2) = force(:,counter2) - f*separation
        end if
        pressure = pressure + f*sep_squared/(3d0*length**3)
     end do
  end do  
